...
from spine_annotator.datasets.sample import SampleIterator, get_random_crop_slice
from spine_annotator.inference.onnx_model import ONNXInferenceModel
from spine_annotator.instance_separation.instance_separation import (
    get_planes_from_rois,
    mask_rare_rois,
    separate_rois_with_labels,
    separate_segmented_rois_by_planes,
)
from spine_annotator.plotting.plot_slice import get_stats, plot_npz, single_plot
from spine_annotator.resources.paths import RAW_NAKO_DATASET_PATH, TRAIN_SPLIT_CSV_PATH, VAL_SPLIT_CSV_PATH
from spine_annotator.spine_types.labels import get_labels_for_n_classes
from spine_annotator.utils.log_dir import get_next_log_dir
from spine_annotator.visualisation.blender.open_in_blender import open_in_blender

colored_tracebacks()

import numpy as np

from spine_annotator.datasets.segmentation_dataset import SegmentationDataModule, expand_path_to_data_dirs
...
def post_process_instances(vertebra_instances, segmentation, plot_dir=None):
    ...
    if plot_dir is not None:
        ...
        from spine_annotator.visualisation.color_palettes import get_alternating_palette
...
def plot_every_val_dataset(directory=None, device=3, model_height=896, cropped_height=None, cropped_V=None):
    ...
    onnx_seg_inference = ONNXInferenceModel.get_new_segmentation_model(device=device)
    # onnx_seg_inference.load_index_list()

    onnx_inst_inference = ONNXInferenceModel.get_best_instance_segmentation_model(px=model_height, device=device)
